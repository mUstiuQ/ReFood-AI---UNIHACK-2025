...
//   await Hive.openBox('donations'); astea ca sa deschidem hive ul pentru salvare
...
class _DonateFoodPageState extends State<DonateFoodPage>
  ...
  Box get _box => Hive.box('donations');

  // ===== API-like (Hive) =====
  Future<List<Map<String, dynamic>>> _listDonations() async {
    final vals = _box.values.toList().cast<Map>();
    // cele mai noi primele, doar 5
    return vals.reversed.take(5).map((e) => Map<String, dynamic>.from(e)).toList();
  }

  Future<void> _createDonation(Map<String, dynamic> data) async {
    // validare după schema ta
  ...
  Widget _buildTypeGrid() {
    final cols = MediaQuery.of(context).size.width >= 900 ? 3 : 1;
  ...
  Widget _buildSuccessCard() {
    return _Surface(
      ...
      child: Column(
        mainAxisAlignment: MainAxisAlignment.center,
        children: const [
          Icon(Icons.check_circle, size: 84, color: Color(0xFF10B981)),
          SizedBox(height: 16),
          Text(
            "Thank You! 🎉",
            style: TextStyle(
              ...
              child: Row(
                children: [
                  Container(
                    ...
                    child: Icon(Icons.fastfood, color: iconColor, size: 22),
                  ),
